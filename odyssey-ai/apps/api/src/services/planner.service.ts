...
import { memoryStore, type TripRecord } from '../lib/mock-data.js';
      ...
      destinationData.lng,
      {
        ...
        constraints: {
          ...
          mobilityRestrictions:
            normalizedInput.constraints?.mobilityRestrictions ?? false,
          dietaryRestrictions:
            normalizedInput.constraints?.dietaryRestrictions ?? [],
  ...
  private buildItinerary(
    ...
    return Array.from({ length: days }, (_, dayIndex) => {
      const selected = activities
        ...
        .map((activity, activityIndex) => {
          ...
          endTime.setUTCMinutes(
            endTime.getUTCMinutes() + activity.durationMinutes,
          );
      ...
      return {
        dayIndex,
        date: new Date(
          new Date(input.startDate).getTime() + dayIndex * 24 * 60 * 60 * 1000,
        )
          .toISOString()
          .slice(0, 10),
        theme:
          dayIndex % 2 === 0 ? 'Calm cultural discovery' : 'Local pulse and social energy',
        mood: input.mood,
        activities: selected,
      };
    });
