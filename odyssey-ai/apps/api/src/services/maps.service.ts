...
import { buildPulseZones } from '../lib/mock-data.js';
...
export class MapsService {
  async getDestinationContext(destination: string): Promise<DestinationContext> {
    return {
      ...
      country: 'Thailand',
      city: 'Bangkok',
      streetViewUrl: `https://maps.googleapis.com/maps/api/streetview?size=800x400&location=13.7563,100.5018&key=${process.env.GOOGLE_MAPS_API_KEY ?? ''}`,
    };
  }
  ...
  scoreMoodFit(activity: ItineraryActivity, mood?: MoodVector): number {
    ...
    const calmBoost =
      activity.moodTags.includes('calm') && mood.anxiety > 0.6 ? 0.18 : 0;
    const energyBoost =
      activity.moodTags.includes('energetic') && mood.energy > 0.6 ? 0.15 : 0;
